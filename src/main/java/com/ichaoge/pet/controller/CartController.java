...
public class CartController extends BaseController {
    ...
    @ResponseBody
    public RemoteResult<?> queryAll(HttpServletRequest request, @RequestBody Cart param) {
        logger.info("请求地址：" + request.getRequestURI() + ",请求参数："+param + "，sessionid:" + request.getSession().getId() + "，用户：" + getUser());
        List<Cart> cartList = null;
        //开始查询
        try {
            cartList = cartServiceI.selectByExample(new CartExample());

            logger.info("应答参数：" + cartList + "sessionid:" + request.getSession().getId() + "用户：" + getUser());
        } catch (Exception e) {
            logger.error("查询列表失败!", e);
            return Utils.webResult(false, ResulstCodeEnum.SERVICE_EXCEPTION.getCode(),"查询列表失败!", null);
        }
        return Utils.webResult(true, ResulstCodeEnum.SERVICE_SUCESS.getCode(),ResulstCodeEnum.SERVICE_SUCESS.getCodeDesc(), cartList);
    }
